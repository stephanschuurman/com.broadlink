...
class BroadlinkPayloadPacket {
   ...
   *
   * Parsing is driven by the length field alone, like the device itself:
   * anything after the length-covered pulse data (an optional 0x0d 0x05
   * trailer, AES padding on raw captures) is ignored. RM4/RM5 captures end
   * exactly at the length field, without a trailer.
   *
   * @param {Uint8Array} bytes  Raw wire-format packet
   * @returns {BroadlinkPayloadPacket}
   * @throws {Error} when the packet is too short or the length field exceeds the data
   */
  static fromUint8Array(bytes) {
    ...
    if (pulseLen === 0 || bytes.length < 4 + pulseLen) throw new Error('Packet length mismatch');
