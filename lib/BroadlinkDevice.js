...
class BroadlinkDevice extends Homey.Device {
  ...
  async onInit(dev) {
    ...
    if (deviceSettings.discovery_interval && deviceSettings.ip_assignment_mode === "automatic")
    ...
    if (changedKeys.length > 0) {
      ...
      changedKeys.forEach(key => {
        ...
        if (key === "discovery_interval" || key === "ip_assignment_mode") {
          ...
          if (newSettings.discovery_interval && newSettings.ip_assignment_mode === "automatic") {
  ...
  async reDiscoverIpAddress(timeout = 3000) {
    ...
    try {
      ...
      if (deviceSettings.ip_assignment_mode === "automatic" && this._communicate)
