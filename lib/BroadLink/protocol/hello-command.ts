...
/**
 * Builds the UDP broadcast packet used to discover Broadlink devices
 */
export class HelloCommandPacket extends BroadlinkPacketBase {

  readonly header: BroadlinkHeader;
  readonly payload: Buffer; // Hello command has no payload
  ...
  constructor(localIp: string, srcPort: number, now: Date = new Date()) {
    ...
    this.header = {
      ...
      errorCode:   0,
      deviceType:  0,
      payloadType: PayloadType.Command.Hello,
      packetCount: 0,
