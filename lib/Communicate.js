...
class Communicate {
  ...
  async discover(timeout, local_ip_address, device_ip_address) {
    ...
    return new Promise((resolve, reject) => {
      ...
      sock.on("message", (msg, rinfo) => {
        this._utils.debugLog(this, `<== Communicate.discover - response from ${rinfo.address}`);
        results.push({
  ...
  async _send(command, data = new Uint8Array()) {
    this._utils.debugLog(this, ">> Sending a packet to the device. <<");
    var payload = new Uint8Array(16);
    payload[0] = command;
    await this.send_packet(0x6a, null, data);
  }

  async send_data(data = new Uint8Array()) {
    this._send(0x2, data);
  }
